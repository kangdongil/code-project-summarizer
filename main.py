...
def write_project_tree_summary(root_dir, output_file, exclude_extensions=None, max_files=4, top_files=3, is_compact=False):
    if exclude_extensions is None:
        exclude_extensions = []

    with open(output_file, 'w', encoding='utf-8') as f:
        f.write("## Project Structure\n")
        tree_structure = generate_project_tree(root_dir, exclude_extensions=exclude_extensions, max_files=max_files, top_files=top_files, is_compact=is_compact)
        f.write(f"```\n{tree_structure}```\n\n")

def write_file_contents(root_dir, output_file, patterns, ignore_dirs, is_compact=False):
    file_contents = get_file_contents(root_dir, patterns, ignore_dirs, is_compact)
...
if __name__ == "__main__":
    ...
    is_compact = True  # Set to True for compact mode

    write_project_tree_summary(current_directory, output_md_file, exclude_extensions=exclude_extensions, is_compact=is_compact)
    write_file_contents(current_directory, output_md_file, inspection_patterns, ignore_directories, is_compact=is_compact)